...
function calculateRecommendation() {
    ...
    btn.addEventListener('click', function () {
        ...
        if (usersSelected != null && usersSelected.length > 1) {
            generateRecommendationResult(usersSelected);
        } else {
...
function generateRecommendationResult(usersSelected) {
    ...
    updateUi(usersSelected, selMGF[0], selAS[0], selFS[0], selDF[0]);
}
...
function updateUi(usersSelected, selMGF, selAS, selFS, selDF) {
    ...
    inner += '</ul>';
    inner += '<p>According with people selected, <span>The Ideal Restaurant Experience</span> ';
    if (selMGF != null) {
    ...
    document.querySelector('.main-restaurant').innerHTML += '<a href="/6" class="button reverse">RE-INITIALIZE RECOMMENDATION</a>'
